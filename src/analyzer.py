...
def combine_results_to_csv(results, output_path):
    ...
    with open(output_path, 'w', newline='') as csvfile:
        ...
        # Write header row
        writer.writerow(["Filepath", "Filename", "Version", "Section", "Metric", "Value"])

        # Write data for each file
        for filename, result in results:
            # Add LOC metrics
            for key, value in result.loc_metrics.items():
                writer.writerow([result.filepath, filename, result.version, "LOC Metrics", key, f"{value:.2f}" if isinstance(value, float) else str(value)])

            # Add Halstead metrics
            for key, value in result.halstead_metrics.items():
                writer.writerow([result.filepath, filename, result.version, "Halstead Metrics", key, f"{value:.2f}" if isinstance(value, float) else str(value)])

            # Add keyword frequency
            for key, value in result.keyword_frequency.items():
                writer.writerow([result.filepath, filename, result.version, "Keyword Frequency", key, f"{value:.2f}" if isinstance(value, float) else str(value)])

            # Add average line length
            writer.writerow([result.filepath, filename, result.version, "General", "Average Line Length", f"{result.avg_line_length:.2f}"])

            # Add score and grade
            writer.writerow([result.filepath, filename, result.version, "Results", "Final Score", result.score])
            writer.writerow([result.filepath, filename, result.version, "Results", "Grade", result.grade])
